...
- ✅ **Двери / краны / выключатели** — игрок 2 открывает их кнопкой **X**
  (через настоящие PlayMaker-события MSC)
- 🧪 **Предметы (эксперимент):** игрок 2 берёт физические предметы своим
  захватом (бутылки, инструменты). «Ручные» предметы игры (шланг заправки
  и т.п.) пока не берутся — они завязаны на единственную руку игрока 1
